...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      localStorage.setItem('stitchflow_token', data.token);
      localStorage.setItem('stitchflow_user', JSON.stringify(data.user));
      navigate('/');
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-navy">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Контроль каждого стежка</p>
        </div>
        ...
        <p className="text-xs text-gray-400 text-center mt-6">
          Тестовый доступ: admin@stitchflow.local / admin123
        </p>
      </div>
    </div>
  );
