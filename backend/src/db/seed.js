...
async function seed() {
  ...
  try {
    ...
    for (let i = 0; i < STAGES.length; i++) {
      ...
      const daysAgo = STAGES.length - (i + 1);
      const startedAt = status !== 'pending' ? new Date(Date.now() - (daysAgo + 1) * 86400000) : null;
      const completedAt = status === 'done' ? new Date(Date.now() - daysAgo * 86400000) : null;
      await client.query(
